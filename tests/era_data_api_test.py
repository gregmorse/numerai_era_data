...
def test_get_current_era_features_no_update(manage_cache):
    instance = manage_cache
    instance._get_data_sources = MagicMock(return_value=[MockDataSource])
    instance.data_cache = pd.DataFrame({"era": ["0001"], "column1": [1]})

    with patch("numerai_era_data.date_utils.get_current_era", return_value=1):
        df = instance.get_current_era_features()
    
    assert df["era"].tolist() == ["0001"]


def test_get_current_era_features_with_update(manage_cache):
    instance = manage_cache
    instance._get_data_sources = MagicMock(return_value=[MockDataSource])
    instance.data_cache = pd.DataFrame({"era": ["0001"], "column1": [1]})
...
def test_get_current_era_features_with_update_and_no_cache(manage_cache):
    instance = manage_cache
    instance._get_data_sources = MagicMock(return_value=[MockDataSource])
    instance.data_cache = pd.DataFrame()
...
def test_get_current_era_features_columns_changed(manage_cache):
    instance = manage_cache
    instance._get_data_sources = MagicMock(return_value=[MockDataSource])
...
def test_get_current_era_features_no_update_stale(manage_cache):
    instance = manage_cache
    instance._get_data_sources = MagicMock(return_value=[MockDataSource])
    instance.data_cache = pd.DataFrame({"era": ["0001"], "column1": [1]})

    with patch("numerai_era_data.date_utils.get_current_era", return_value=3):
        df = instance.get_current_era_features(False)
    
    assert df["era"].tolist() == ["0001"]


def test_get_all_era_features_no_update(manage_cache):
    instance = manage_cache
    instance._get_data_sources = MagicMock(return_value=[MockDataSource])
    instance.data_cache = pd.DataFrame({"era": ["0001"], "column1": [1]})

    with patch("numerai_era_data.date_utils.get_current_era", return_value=1):
        df = instance.get_all_era_features()
    
    assert df["era"].tolist() == ["0001"]


def test_get_all_era_features_with_update(manage_cache):
    instance = manage_cache
    instance._get_data_sources = MagicMock(return_value=[MockDataSource])
    instance.data_cache = pd.DataFrame({"era": ["0001"], "column1": [1]})
...
def test_get_all_era_features_with_update_and_no_cache(manage_cache):
    instance = manage_cache
    instance._get_data_sources = MagicMock(return_value=[MockDataSource])
    instance.data_cache = pd.DataFrame()
...
def test_get_all_era_features_columns_changed(manage_cache):
    instance = manage_cache
    instance._get_data_sources = MagicMock(return_value=[MockDataSource])
    instance.data_cache = pd.DataFrame({"era": ["0001"], "column0": [1]})
...
def test_get_all_era_features_no_update_stale(manage_cache):
    instance = manage_cache
    instance._get_data_sources = MagicMock(return_value=[MockDataSource])
    instance.data_cache = pd.DataFrame({"era": ["0001"], "column1": [1]})

    with patch("numerai_era_data.date_utils.get_current_era", return_value=3):
        df = instance.get_all_era_features(False)
    
    assert df["era"].tolist() == ["0001"]


def test_get_current_daily_features_no_update(manage_cache):
    instance = manage_cache
    instance._get_data_sources = MagicMock(return_value=[MockDataSource])
    ...
    with patch("numerai_era_data.date_utils.get_current_date", return_value=data_date):
        df = instance.get_current_daily_features()
    
    assert df[BaseDataSource.DATE_COL].tolist() == [data_date]
    assert df["column1"].tolist() == [1]


def test_get_current_daily_features_with_update(manage_cache):
    instance = manage_cache
    instance._get_data_sources = MagicMock(return_value=[MockDataSource])
...
def test_get_current_daily_features_with_update_and_no_cache(manage_cache):
    instance = manage_cache
    instance._get_data_sources = MagicMock(return_value=[MockDataSource])
    data_date = date(2001, 4, 20)
    instance.daily_cache = pd.DataFrame()
...
def test_get_current_daily_features_no_update_stale(manage_cache):
    instance = manage_cache
    instance._get_data_sources = MagicMock(return_value=[MockDataSource])
    data_date = date(2022, 1, 1)
    ...
    with patch("numerai_era_data.date_utils.get_current_date", return_value=request_date):
        df = instance.get_current_daily_features(False)
